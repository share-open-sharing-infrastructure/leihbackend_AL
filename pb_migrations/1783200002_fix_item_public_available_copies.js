...
migrate((app) => {
  ...
  const newCollection = new Collection({
    ...
    "type": "view",
    "updateRule": null,
    "viewQuery": "SELECT item.id, item.iid, item.name, item.description, item.status, item.deposit, item.images, item.synonyms, item.category, item.brand, item.model, item.packaging, item.manual, item.parts, item.copies, item.added_on, item.is_protected, (item.copies - COALESCE((SELECT SUM(COALESCE((SELECT CAST(jrc.value AS INTEGER) FROM json_each(COALESCE(NULLIF(r.requested_copies, ''), '{}')) jrc WHERE jrc.key = item.id LIMIT 1), 1)) FROM reservation r WHERE r.done = 0 AND EXISTS (SELECT 1 FROM json_each(r.items) ji WHERE ji.value = item.id)), 0) - COALESCE((SELECT SUM(COALESCE((SELECT CAST(jrn.value AS INTEGER) FROM json_each(COALESCE(NULLIF(rn.requested_copies, ''), '{}')) jrn WHERE jrn.key = item.id LIMIT 1), 1)) FROM rental rn WHERE (rn.returned_on = '' OR rn.returned_on IS NULL) AND EXISTS (SELECT 1 FROM json_each(rn.items) ji WHERE ji.value = item.id)), 0)) AS available_copies FROM item WHERE item.status != 'deleted'",
    "viewRule": ""
  })
